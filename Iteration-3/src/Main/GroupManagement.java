...
public class GroupManagement extends Thread{
	...
	GroupManagement(String tName, DatagramSocket outgoingSocket, String ip, int port, Hashtable<Source, Vector<Peer>> listOfSources, Vector<UDPMessageLog> peersSent){
		...
		System.out.println("Created GroupManagement Thread!");
	}
	...
	private void checkActivity(int inactivity_max) {
		...
		for (Map.Entry<Source, Vector<Peer>> s : listOfSources.entrySet()) {
			...
			for (Peer peer : s.getValue()) {
				...
				//Inspired by: https://stackoverflow.com/questions/4927856/how-can-i-calculate-a-time-difference-in-java
	...
	//Function for sending a UDPMessage via the socket we created in main and sends a "peer" message to the peer param
